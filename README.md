...
- **`apis/`** - One OpenAPI source per surface: `v2.yaml` (the `/api/v2` API), `widget.yaml`, `teams.yaml`, `rt.yaml`
- **`components/common.yaml`** - Shared security schemes + RFC-7807 `Problem` + standard error responses, `$ref`d by every spec
- **`schemas/`** - Promoted JSON-schema data models (generated from `data_models/`), referenced by specs via `../schemas/` relative refs
- **`dist/`** - Bundled (self-contained) specs, one per API, regenerated by `scripts/bundle-all.sh` (registry-driven from `redocly.yaml`)
- **`redocly.yaml`** - The API registry; lint and bundling derive the spec list from here, so adding an API needs no workflow edits
...
```sh
./scripts/bundle-all.sh   # bundles every API in redocly.yaml to dist/
```
